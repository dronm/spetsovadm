//----- template ----- //client pay documents
<template>
	<GridView
		grid-key="ClientPayList"
		:key-fields="['id']"
		:title="$t('ClientPayList.title')"
...
import { clientPayList } from '../services/clientPay';
...
const columns = ref<GridCol[][]>([
	[
		{
			id: 'num_1c',
			header: t('ClientPayList.columns.num_1c'),
			field: {
...
gridStore.setSrvOperations(<GridSrvOperations>{
	[GridSrvOperation.list]: clientPayList,
});
</script>
